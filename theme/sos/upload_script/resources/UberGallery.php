...
class UberGallery {
     ...
     * @access public
     */
    public function readImageDirectory($directory, $view = '') {
        ...
        if (!$galleryArray) {
            ...
            foreach ($dirArray as $key => $image) {                        
                ...
                if($view == 'front')
                {
                    $new_w = 100;
                    $new_h = 100;
                    $system=explode(".",$image['real_path']);

                    if (preg_match("/jpg|jpeg/",$system[1])){$src_img=imagecreatefromjpeg($image['real_path']);}
                    if (preg_match("/png/",$system[1])){$src_img=imagecreatefrompng($image['real_path']);}
                    if (preg_match("/gif/",$system[1])){$src_img=imagecreatefromgif($image['real_path']);}
                    ...
                    $galleryArray['images'][htmlentities(pathinfo($image['real_path'], PATHINFO_BASENAME))] = array(
                        ...
                        'thumb_path'   => $this->_createThumbnail($image['real_path'], $thumb_w, $thumb_h)
                    );
                }
                else
                {
     ...
     * @access private
     */
    public function _createThumbnail($source, $thumbWidth = NULL, $thumbHeight = NULL, $quality = NULL, $save_path_img = '', $retailer_id = '') {
        ...
        {
            $image_name = explode('/', $source);
            $name_img = $image_name[count($image_name) - 1]; 
            $nme = explode('.', $name_img);
        }
        ...
        // Build file name
        if(isset($name_img))
            $fileName = $thumbWidth . 'x' . $thumbHeight.'_'.$nme[0] . '.' . $fileExtension;
        else
        ...
        if($save_path_img == '')
            ...
            $destination = $save_path_img . '/' . $nme[0].'.'.$fileExtension;
