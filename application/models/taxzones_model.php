<?php 
class Taxzones_Model extends Isp_Model 
{
	...
	{
		$qry = 'select taxzone_ID from county_taxzone_mapping where LOWER( county_taxzone_mapping.county ) = "'.strtolower($County).'"';
        	$res = $this->db->query($qry);
        	$tax_code = $res->result_array();
		return $tax_code;
	}
	
	public function updateTaxZones($result)
	{
		if(count($result)>0)
		{
			$u=0;
			...
			{
				$taxzone_id = $res['taxzone_ID'];
				$taxzone_name = $res['taxzone_name'];
				
				$isExist = $this->isTaxzoneExist($taxzone_id, $taxzone_name);
				if($isExist==true)
				{
					$data = array(
					   'taxzone_name' => $taxzone_name
					);

					$this->db->where('taxzone_ID', $taxzone_id);
					$this->db->update('county_taxzone_mapping', $data); 
					$u++; 
				}
				else
				{
					...
					$this->db->insert('county_taxzone_mapping', $data); 
					$i++;
				}
			}
			$return['inserted']=$i;
	...
	function isTaxzoneExist($taxzone_id, $taxzone_name)
	{
		$query = $this->db->get_where('county_taxzone_mapping', array('taxzone_id' => $taxzone_id));
		if ($query->num_rows() > 0)
		{
			return true;
		} 
